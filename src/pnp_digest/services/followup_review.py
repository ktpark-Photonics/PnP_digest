"""followup manifest 수동 확인 CSV export/import 유틸리티."""
...
FOLLOWUP_REVIEW_REQUIRED_COLUMNS = {
    "run_id",
    "followup_team",
    "blocked_reason",
    "review_task_id",
...
def default_followup_review_export_path(source_followup_manifest_path: Path) -> Path:
    """입력 followup manifest 기준 기본 CSV 경로를 만든다."""

    return source_followup_manifest_path.with_name("ops_daily_queue.csv")


def export_followup_review_manifest(
    manifest: OpsFollowupManifest,
    *,
    source_followup_manifest_path: Path,
    output_path: Path | None = None,
) -> Path:
    """followup manifest를 사람이 수정할 CSV 큐로 저장한다."""

    resolved_output_path = output_path or default_followup_review_export_path(source_followup_manifest_path)
...
def _validate_columns(fieldnames: list[str] | None) -> None:
    ...
    missing_columns = sorted(FOLLOWUP_REVIEW_REQUIRED_COLUMNS - actual_columns)
    if missing_columns:
        raise ValueError(
            "followup review import CSV에 필요한 컬럼이 없습니다: " + ", ".join(missing_columns)
        )
...
def _task_map(manifest: OpsFollowupManifest) -> dict[str, ReviewTask]:
    """task ID 기준 원본 task 맵을 만든다."""

    return {task.review_task_id: task for task in manifest.tasks}


def _validate_readonly_columns(manifest: OpsFollowupManifest, rows: list[dict[str, str]]) -> None:
    """CSV의 읽기 전용 컬럼이 원본 followup manifest와 일치하는지 확인한다."""
    ...
    for row in rows:
        ...
        if task_id in seen_task_ids:
            raise ValueError(f"followup review import CSV에 중복 task가 있습니다: {task_id}")
        seen_task_ids.add(task_id)

        if task_id not in task_map:
            raise ValueError(f"followup review import CSV에 원본 manifest에 없는 task가 있습니다: {task_id}")

        task = task_map[task_id]
        expected_values = {
            "run_id": manifest.run_id,
            "followup_team": manifest.followup_team,
        ...
        if mismatched_columns:
            raise ValueError(
                "followup review import CSV의 읽기 전용 컬럼이 원본 manifest와 다릅니다: "
                + ", ".join(mismatched_columns)
            )


def build_ops_followup_resolution_artifact(
    manifest: OpsFollowupManifest,
    *,
    source_followup_manifest_path: Path,
    review_csv_path: Path,
) -> OpsFollowupResolutionArtifact:
    """사람이 수정한 followup review CSV를 JSON artifact로 변환한다."""
    ...
    if manifest.tasks and len(rows) != len(manifest.tasks):
        raise ValueError("followup review import CSV row 수가 원본 followup task 수와 다릅니다.")
    if not manifest.tasks and rows:
        raise ValueError("원본 followup manifest에 task가 없으므로 CSV에도 데이터 행이 있으면 안 됩니다.")
    ...
    return OpsFollowupResolutionArtifact(
        run=manifest.run,
        run_id=manifest.run_id,
        source_followup_manifest_path=str(source_followup_manifest_path),
