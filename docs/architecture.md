...
- `retry`: publish review 결과에서 실패 또는 미확인 채널만 추려 후속 재배포 대상을 정리하는 단계
- `handoff`: retry 결과를 운영팀 전달용 task artifact로 정리하는 단계
- `handoff review`: 운영팀이 handoff task 상태를 다시 artifact로 반영하는 단계
- `followup`: handoff resolution 이후에도 남아 있는 open/in_review task를 다시 묶는 단계
- `followup review`: followup CSV 큐를 다시 artifact로 반영하는 단계
- `escalation`: followup resolution 이후에도 `in_review`로 남아 있는 task를 다시 모으는 단계
- `escalation review`: escalation CSV 큐를 다시 artifact로 반영하는 단계
- `closure`: escalation resolution을 기준으로 종결 task와 남은 task를 함께 정리하는 단계
- `closure review`: closure CSV를 다시 artifact로 반영하는 단계
