...
현재 구현 범위는 `Phase 4.3`이며 다음을 포함한다.
...
Phase 4.3 현재 범위에서는 `assess-relevance`, `verify`, `review import/export`, `summarize`, `explain`, `render`를 구현했으며, 현재 `render`는 Markdown, DOCX, PDF, PPTX를 지원한다.
